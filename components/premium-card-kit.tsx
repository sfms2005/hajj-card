      ...
      {cards.map((card) => {
        ...
        return (
          <button
            ...
            className={`group relative aspect-[3/4] w-[calc(33.333%-0.5rem)] min-w-[4.75rem] max-w-[6.75rem] overflow-hidden rounded-[22px] text-right transition duration-300 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent sm:min-w-[5.5rem] sm:max-w-[7.5rem] sm:rounded-[24px] ${
              isSel
            ...
            style={{
              boxShadow: isSel
                ...
                : `0 0 0 1px color-mix(in srgb, ${t.accent} 28%, transparent)`,
            }}
          >
            <span
            ...
            <span
              ...
              style={{
                background: `linear-gradient(165deg, rgba(0,0,0,0.38) 0%, color-mix(in srgb, ${t.accent} 42%, rgba(0,0,0,0.62)) 100%)`,
              }}
              aria-hidden
            />
            <span className="relative z-10 flex h-full min-h-0 items-center justify-center overflow-hidden p-1.5 sm:p-2.5">
              <span className="text-center text-[0.55rem] font-semibold leading-[1.35] text-balance text-white drop-shadow-md sm:text-[0.62rem]">
                {card.text}
              </span>
            </span>
          </button>
...
>(function PremiumCardPreview({ card, name }, ref) {
  ...
  return (
    <div
      ...
      style={{
        boxShadow: `0 0 0 3px color-mix(in srgb, ${t.accent} 55%, transparent), 0 24px 56px -14px rgba(15, 23, 42, 0.38)`,
      }}
    >
      <div
      ...
      <div
        ...
        style={{
          background: `linear-gradient(170deg, rgba(0,0,0,0.38) 0%, color-mix(in srgb, ${t.accent} 38%, rgba(0,0,0,0.58)) 100%)`,
        }}
        aria-hidden
      />
      <div className="relative z-10 flex h-full min-h-0 flex-col items-center justify-center px-4 py-7 text-center text-white sm:px-6 sm:py-9">
        ...
        {name.trim() ? (
          <p
            ...
            style={{
              color: `color-mix(in srgb, white 92%, ${t.accent} 8%)`,
            }}
          >
            {name.trim()}
          </p>
        ) : (
          <p className="mt-4 min-h-[1.5rem] shrink-0 text-sm text-white/50 sm:mt-5 sm:text-base">
            —
          </p>
        )}
      </div>
